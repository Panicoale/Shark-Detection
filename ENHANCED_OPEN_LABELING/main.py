...
# MODIFICATION TO MOVE THE INPUT DIR INSIDE AN EXTERNAL HARD DISK
HD=0
if HD ==0:
    INPUT_DIR = args.input_dir
else:
    INPUT_DIR = '/Volumes/APOLLO-11/input'
...
def get_next_frame_path_list_limit(video_name, img_path,frames):
    first_index = VIDEO_NAME_DICT[video_name]['first_index']
    last_index = VIDEO_NAME_DICT[video_name]['last_index']
    img_index = IMAGE_PATH_LIST.index(img_path)
    limit = frames+1
    if last_index-img_index < limit:
        return IMAGE_PATH_LIST[(img_index + 1):last_index]
    else:
        return IMAGE_PATH_LIST[(img_index + 1):img_index+limit]
...
def remove_new_objects(object_list, img_path, json_file_data):
    frame_data_dict = json_file_data['frame_data_dict']
    json_object_list = get_json_file_object_list(img_path, frame_data_dict)
    # copy the list since we will be deleting elements without restarting the loop
    temp_object_list = object_list[:]
        ...
        # show edges key listener
            ...
            if is_from_video:
                ...
                if len(object_list) > 0:
                    ...
                    # initial frame
                    init_frame = img.copy()
                    label_tracker = LabelTracker('CSRT', init_frame, next_frame_path_list) # TODO: replace 'KCF' by 'CSRT'
                    control=0
